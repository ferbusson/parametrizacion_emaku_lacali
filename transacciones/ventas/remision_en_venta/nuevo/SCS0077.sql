--SCS0077
--Se ejecuta cuando se reciben parametros con fecha, en la cali esta No se ejecuta
SELECT
    i.orden,
    id.rf_documento,
     CASE WHEN
        ...
        --d.codigo_tipo IN ('TF','TI','TR','TE','TA','TG','TU','TL','IJ','AF','AO')
        d.codigo_tipo IN ('TF','TI','TR','TB','TE','TA','TG','TU','TL')
        ) 
    THEN 'TB'
         ELSE CASE WHEN
         ...
         ELSE CASE WHEN 
                i.entrada IS NOT NULL AND
                d.codigo_tipo IN ('EA','IM','RC','AT','SI','IP') THEN 'EA' 
         ELSE CASE WHEN 
                    i.entrada IS NOT NULL AND
